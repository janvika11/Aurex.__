...
function HomeScreen() {
  ...
  const { isLoading, refetch, dataUpdatedAt, isError, error } = useMarketData();

  useEffect(() => { registerForNotifications(); }, []);

  const lastUpdated = dataUpdatedAt
    ? new Date(dataUpdatedAt).toLocaleTimeString('en-IN')
    : (isError ? 'Connection failed' : 'Fetching...');

  const bottomPadding = insets.bottom + TAB_BAR_HEIGHT + EXTRA_BOTTOM;
  ...
  return (
    ...
    >
      ...
      <Text style={styles.sub}>
        {isError ? 'Cannot reach server. Check API URL in src/config.js and that backend is running.' : 'Pull down to refresh · Auto-updates every 60s'}
      </Text>
      <ForexCard />
      <GoldCard />
    </ScrollView>
